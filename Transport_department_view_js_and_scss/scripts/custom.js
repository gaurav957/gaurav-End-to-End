		...
		<div class="container-fluid nav-container"> \
			<div class="navbar-header"> \
				<div class="dashboardBlock"> \
					<div class="usermetaleftheader">\
						  <ul class="list-inline">\
							<li v-html="user.userdetails.name"></li>\
							<li v-if="user.userdetails.age">Age : {{user.userdetails.age}}</li>\
							<li>{{user.userdetails.gender}}</li>\
							<li>{{user.userdetails.address}}</li>\
						  </ul>\
					</div>\
				</div> \
                <div  class="navbar-brand"> \
                   <a href="#" class="logo"> \
						<img src="https://e2eresearch.com/swteam/html5Tools/texas-department-transportation/img/logo.jpg"  v-if="user.type===\'dashboard\'"> \
					...
					<a v-if="user.type===\'survey\' && user.dashboardlink !== \'\'" href="javascript:void(0)" class="nv-bind dashboardLink" :data-pageid="user.dashboardlink" class="dashboardLink"> Dashboard</a> \
					<div class="user-pic text-center" v-if="user.type===\'dashboard\'"> \
						<img :src="user.userdetails.profile_pic" alt="" class="img-circle"> \
						<div class="user-name" v-html="user.userdetails.name"></div> \
				   </div> \
                </div> \
...
Vue.component('header_panel_new', {
    props: ['user'],
    template: '<div> \
		<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation"> \
		<div class="container-fluid nav-container"> \
			<div class="navbar-header"> \
                <div  class="navbar-brand"> \
                   <a href="#" class="logo"> \
						<img src="https://e2eresearch.com/swteam/html5Tools/texas-department-transportation/img/logo-survey.png"> \
					</a> \
                </div> \
			</div> \
			<div id="navbar" class="collapse navbar-collapse"> \
    ...
    template:'<div id="sidebar-wrapper"> \
        <nav id="spy"> \
            <ul class="sidebar-nav nav"> \
                <li v-for="(index,member) in members"> \
                    <a href="javascript:void(0)" v-on:click="getmember(member.id)"> \
                    	<span v-html="member.name"></span><span v-html="member.id"></span> \
						<span class="plus" v-if="user.type===\'survey\'">&nbsp;</span> \
                    	<i :class="{\'icon\':true, \'inprogress\':member.status==\'In-progress\',\'completed\':member.status==\'Completed\',\'notstarted\':member.status==\'Not completed\'}" v-if="user.type===\'dashboard\'"></i> \
                    </a> \
					<ul class="list-unstyled submenu" v-if="user.type===\'survey\'"> \
						<li v-if="submenu.status!==\'99\'" v-for="submenu in member.sublink" :class="{\'current\':submenu.Page_id == user.current_page}"><a :href="submenu.link" :data-pageid="submenu.Page_id" :class="{\'status-icon nv-bind\':true,\'inprogress\':submenu.status==\'In-progress\',\'completed\':submenu.status==\'Completed\',\'notstarted\':submenu.status==\'Not started\'}" :data-status="submenu.status">{{submenu.title}}<span class="line">&nbsp;</span></a></li> \
					</ul> \
                </li> \
            </ul> \
        </nav> \
        <ul :class="{\'menu-legend\':user.type==\'dashboard\',\'status-label list-inline\':user.type==\'survey\'}"> \
			<li :class="{\'inprogress\':user.type===\'survey\'}"><i class="icon inprogress" v-if="user.type===\'dashboard\'"></i> In-progress</li> \
			<li :class="{\'completed\':user.type===\'survey\'}"><i class="icon completed" v-if="user.type===\'dashboard\'"></i> Completed</li> \
			<li :class="{\'notstarted\':user.type===\'survey\'}"><i class="icon notstarted" v-if="user.type===\'dashboard\'"></i> Not started</li> \
		</ul> \
    </div>',
	methods: {		
                    ...
                    <div class="col-md-12 mr-bot-15"> \
                       	<div class="white-resize"> \
                       		<div class="panel panel-default shadow"> \
								...
								<div class="panel-body dashbord-user-details"> \
									<a v-on:click="punchuserid()" href="javascript:void(0)" :data-pageid="members.id" class="btn text-uppercase btn-success start-btn pull-right">Start</a>\
									<ul class="list-inline user-details"> \
										<li v-if="members.name"><h2 class="house-member">{{members.name}}</h2></li> \
										<li v-if="members.age">Age : <span v-html="members.age"></span></li> \
										<li v-if="members.gender">Gender : <span v-html="members.gender"></span></li> \
										<li v-if="members.active_members">Active member : <span v-html="members.active_members"></span></li> \
										<li v-if="members.status">Status : <span v-html="members.status"></span></li> \
									</ul> \
								</div> \
							</div> \
                       	</div> \
...
var vueObj = new Vue({
	...
	created: function() {
		...
		$(document).on('click','.answerlabel',function(){
			$(".temp-br").remove();
			setTimeout(function(){ $(".temp-br").remove(); }, 500);
		});
